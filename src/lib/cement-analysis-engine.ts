...
function buildDocumentDrivenChecks(
  ...
  if (fluidLoss.length > 0) {
    ...
    // Нормативы: чистый ЦР (≥1.65 г/см³) — до 50 мл/30мин норма; облегчённый (<1.65) — до 100 мл/30мин норма
    const hasHeavySlurry = slurries.some(s => s.density >= 1.65);
    const hasLightSlurry = slurries.some(s => s.density < 1.65);
    const normHeavy = 50;
    const normLight = 100;
    const effectiveNorm = hasLightSlurry ? normLight : normHeavy;

    // Проверка на подозрительно низкие плановые значения (< 10 мл — скорее всего ошибка в документе)
    const suspiciouslyLow = fluidLoss.filter(v => (v.numeric || 0) < 10 && (v.numeric || 0) > 0);
    let suspiciousNote = "";
    if (suspiciouslyLow.length > 0) {
      suspiciousNote = ` ⚠ **Внимание**: значение ${suspiciouslyLow.map(v => `${v.raw}`).join(", ")} представляется нереалистично низким. ` +
        `По нормативам ГОСТ 1581, API RP 10B-2 и отраслевой практике типичные значения водоотдачи составляют: ` +
        `для чистых тампонажных растворов (ρ ≥ 1.65 г/см³) — до **${normHeavy} мл/30мин**, ` +
        `для облегчённых (ρ < 1.65 г/см³) — до **${normLight} мл/30мин**. ` +
        `Вероятнее всего, указанное в документе значение является ошибочным.`;
    }

    checks.push({ section: "Сверка с документами", title: `Водоотдача — лабораторные данные`, status: avgFL <= effectiveNorm ? "ok" : "warning",
      detail: `Из документов извлечены значения водоотдачи: ${fluidLoss.map(v => `**${v.raw}**`).join(", ")}. ` +
        (avgFL <= normHeavy ? `Водоотдача ≤ ${normHeavy} мл/30 мин — в пределах нормы для чистого тампонажного раствора.` :
         avgFL <= normLight ? `Водоотдача в пределах нормы для облегчённого тампонажного раствора (≤ ${normLight} мл/30 мин).${hasHeavySlurry ? " Для чистого ЦР (ρ ≥ 1.65 г/см³) рекомендуется < 50 мл/30 мин." : ""}` :
         `Повышенная водоотдача (> ${effectiveNorm} мл/30 мин) увеличивает риск преждевременного схватывания, образования мостов и неполного заполнения затрубного пространства. Рекомендуется добавление понизителя водоотдачи.`) +
        suspiciousNote });
  }
