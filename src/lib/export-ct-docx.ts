...
export async function exportCTDocx(input: CTDocxInput) {
  ...
  if (chartImages?.hydraulics) {
    ...
    children.push(new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { before: 200, after: 200 },
      children: [new ImageRun({ data: buffer, transformation: { width: 550, height: 300 }, altText: { title: "Hydraulics", description: "Pressure drop vs flow rate", name: "hydraulics-chart" } })],
    }));
  }
