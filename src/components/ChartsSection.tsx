          ...
          <ScrollableChart chartRef={chartRef1} height="h-[550px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={pressureData} margin={{ top: 20, right: 115, left: 25, bottom: 30 }}>
                ...
                <YAxis yAxisId="density" orientation="right" domain={[(dataMin: number) => Math.max(0, Math.floor((dataMin - 0.02) * 50) / 50), (dataMax: number) => Math.ceil((dataMax + 0.02) * 50) / 50]} label={{ value: "ρ, г/см³", angle: 90, position: "insideRight", offset: 5, fontSize: 12 }} className="text-xs" width={55} tick={{ fontSize: 11 }} tickFormatter={(v: number) => v.toFixed(2)} allowDecimals tickCount={8} />
